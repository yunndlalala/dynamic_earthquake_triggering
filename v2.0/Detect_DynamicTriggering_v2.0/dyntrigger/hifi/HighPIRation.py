...
def HighPIRatio(
        ...
        PIdatabase_path,
        out_file):
    file_exist, PIdata_frame = load_PIdata(sta, chn,Tb_B, Te_E, PIdatabase_path)
    if file_exist:
...
def run_HighPIRatio(catalog, PIdatabase_path, sta, chn, out_file):
    ...
    for i in range(length):
        ...
        Tb_B = UTCDateTime(event_dataframe.iloc[i]['Tb_B_time'])
        Tb_E = UTCDateTime(event_dataframe.iloc[i]['Tb_E_time'])
        Te_B = UTCDateTime(event_dataframe.iloc[i]['Te_B_time'])
        Te_E = UTCDateTime(event_dataframe.iloc[i]['Te_E_time'])
